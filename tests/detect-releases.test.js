...
test("detector creates an independent matrix only for advanced scripts", () => {
  ...
  released.set(
    ...
    sources.get("sjtu-course-assistant-plus").replace("@version      0.9.0", "@version      0.8.2"),
  );
  ...
  assert.deepEqual(result.candidates, [{ script_id: "sjtu-course-assistant-plus", version: "0.9.0" }]);
